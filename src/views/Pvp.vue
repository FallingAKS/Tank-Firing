  ...
  methods: {
    ...
    Ground() {
      ...
      this.draw=function(keys){
        ...
        ctx.stroke();
        if(win==='none')redOne.move(rv,keys[68],keys[65],keys[87],keys[83]);
        // if(win==='none')redOne.move(rv,keys[65],keys[68],keys[87],keys[83]);
        redOne.draw(rg);
        if(keys[81])redOne.fire(addBullet);
        if(win==='none')greenOne.move(rv,keys[39],keys[37],keys[38],keys[40]);
        greenOne.draw(rg);
    ...
    Page() {
      ...
      // 如果Page函数需要访问DOM，可以考虑使用Vue的ref代替getElementById
    },
    initPage() {
      // 原来window.onload中的逻辑
      this.Page.init(this.$el); // 假设init是Page方法的一部分
    }
  },

  mounted() {
    // 组件挂载后，相当于原来的window.onload
    this.initPage();
  }
};
</script>
