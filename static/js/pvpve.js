...
function easyDirection(moves) {
  // 假设 sensors 对象包含了坦克的传感器信息，例如墙的距离
  let r = Math.random();
  let wallClose = false;
  let wallDirection = 'left';
  const wallDistance = 0.01;

  // if (Math.abs(oldX[iNumber] - newX[iNumber]) < Math.abs(oldY[iNumber] - newY[iNumber])) {
  if (Math.abs(oldX[iNumber] - newX[iNumber]) < wallDistance) {
    wallClose = true;
    wallDirection = 'left';
  }
  // } else {
  else if (Math.abs(oldY[iNumber] - newY[iNumber]) < wallDistance) {
    wallClose = true;
    wallDirection = 'right';
  }
  // }
  ...
  // 如果坦克接近墙壁，那么转弯
  if (wallClose) {
    // 假设 sensors.wallDirection 可以告诉我们墙在左边还是右边
    if (wallDirection === 'left') {
      moves.right = true; // 墙在左边，向右转
    } else {
      moves.left = true; // 墙在右边，向左转
    }
  }
  ...
  // 如果没有靠近墙壁，使用随机逻辑，但增加前进和后退的概率
  if (r < 0.45) {
    moves.go = true; // 增加前进的概率
    iMoves[iNumber] = 'go';
    iPeriod[iNumber] = r / 2;
  } else if (r < 0.6) {
    moves.back = true; // 增加后退的概率
    iMoves[iNumber] = 'back';
    iPeriod[iNumber] = r - 0.45;
  } else if (r < 0.75) {
    moves.left = true; // 增加左转的概率
    iMoves[iNumber] = 'left';
    iPeriod[iNumber] = r - 0.6;
  } else if (r < 0.9) {
    moves.right = true; // 增加右转的概率
    iMoves[iNumber] = 'right';
    iPeriod[iNumber] = r - 0.7;
  } else if (r < 0.95) {
    moves.fire = true; // 开火的概率减少
  }
  // 其余的时间不做任何动作，这样可以让坦克有更多时间前进或后退
...
function hardDirection(moves) {
  ...
  // bfs
  let route = [];
  if (Math.sqrt((blackX - redX) ** 2 + (blackY - redY) ** 2) < Math.sqrt((blackX - greenX) ** 2 + (blackY - greenY) ** 2))
    route = bfs(Math.floor(blackX), Math.floor(blackY), Math.floor(redX), Math.floor(redY));
  else
    route = bfs(Math.floor(blackX), Math.floor(blackY), Math.floor(greenX), Math.floor(greenY));
  ...
  if (route && route.length > 2) {
    ...
    const turnDistance = 0.17;
    const goDistance = 0.21;

    // moves.go = true;
    // moves.right = true;
    // iPeriod[iNumber] = goDistance;
    // iMoves[iNumber] = 'go';

    // 先转向
    if (Math.abs(direction[iNumber]) < 0.1 || Math.abs(direction[iNumber] - 2 * Math.PI) < 0.1) {
      if (nextX > blackX) {
        moves.go = true;
        iPeriod[iNumber] = goDistance;
        iMoves[iNumber] = 'go';
      } else if (nextX < blackX) {
        moves.left = true;
        iPeriod[iNumber] = 2 * turnDistance;
        iMoves[iNumber] = 'left';
      } else if (nextY > blackY) {
        moves.right = true;
        iPeriod[iNumber] = turnDistance;
        iMoves[iNumber] = 'right';
      } else if (nextY < blackY) {
        moves.left = true;
        iPeriod[iNumber] = turnDistance;
    ...
    } else if (Math.abs(direction[iNumber] - Math.PI / 2) < 0.1) {
      if (nextX > blackX) {
        moves.right = true;
        iPeriod[iNumber] = turnDistance;
        iMoves[iNumber] = 'right';
    ...
    } else if (Math.abs(direction[iNumber] - Math.PI) < 0.1) {
      if (nextX > blackX) {
        moves.left = true;
        iPeriod[iNumber] = 2 * turnDistance;
        iMoves[iNumber] = 'left';
    ...
    } else if (Math.abs(direction[iNumber] - 3 * Math.PI / 2) < 0.1) {
      if (nextX > blackX) {
        moves.left = true;
        iPeriod[iNumber] = turnDistance;
        iMoves[iNumber] = 'left';
...
Ground = function () {
  ...
  this.init = function (_canvas, _x, _y, _width, _height) {
    ...
    greenOne = new Tank().init(canvas, t1.x + 0.5, t1.y + 0.5, Math.random() * 2 * Math.PI, 'green');
    blackOnes = [];
    for (let i = 0; i < npcNum; i++) {
  ...
  this.draw = function (keys) {
    ...
    ctx.stroke();
    if (win == 'none') {
      redOne.move(rv, keys[83], keys[70], keys[69], keys[68]);
    ...
    if (win == 'none') {
      greenOne.move(rv, keys[37], keys[39], keys[38], keys[40]);
      greenX = greenOne.getPos().x;
      greenY = greenOne.getPos().y;
    }
    greenOne.draw(rg);
    ...
    for (iNumber = 0; iNumber < blackOnes.length; iNumber++) {
      ...
      if (difficulty !== 2 ||
        Math.sqrt((blackOnes[iNumber].getPos().x - redX) ** 2 + (blackOnes[iNumber].getPos().y - redY) ** 2) < 2 ||
        Math.sqrt((blackOnes[iNumber].getPos().x - greenX) ** 2 + (blackOnes[iNumber].getPos().y - greenY) ** 2) < 2)
        blackOnes[iNumber].draw(rg);
    }
    for (let i = 0; i < bullets.length; i++) {
      if (win == 'none') {
        bullets[i].move(rv);
        if (bullets[i].hit(redOne) || bullets[i].hit(greenOne)) {
          win = 'blacks';
          blackDiedNum = 0;
        }
...
Page = function () {
  ...
  this.draw = function () {
    ...
    ctx.fillText('红色绿色 ' + redCnt, 250, 870);
    ctx.fillText('黑色恶犬 ' + blacksCnt, 750, 870);
    ctx.fillText('ESDF+Q', 400, 870);
    ctx.fillText('电脑控制', 900, 870);
    ctx.fillText('方向键+M', 550, 870);
    ground.draw(keys);
